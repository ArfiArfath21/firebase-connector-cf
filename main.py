...
def firebase_connector(request):
    ...
    if request.method == "POST":
            ...
            if "update_dict" in request_json:
                if isinstance(request_json["update_dict"], dict):
                    ...
                    try:
                        ...
                        return (
                            {
                                "message": f"Incompatible update_dict: {err}\n{request_json['update_dict']}"
                            },
                            400,
                            headers,
                        )
        ...
        if request_json["request_type"] == "deleteUser":
            ...
            return (
                {"message": f"User {request_json['user_id']} Deleted"},
                200,
                headers,
            )
        ...
        if request_json["request_type"] == "addToLeaderboard":
            ...
            for winner_id, winner in enumerate(
                request_json["update_dict"]["winner_data"]
            ):
                leaderboard_doc[f"winner_{winner_id+1}"] = winner

            doc = collection.document(
                f"{leaderboard_doc['activity']}_{leaderboard_doc['startTime']}"
            )
